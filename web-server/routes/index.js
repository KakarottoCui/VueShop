...
router.post('/api/add_order', (req, res) => {
    ...
    //保存收货地址到用户
    let update_user_addr_sql = "UPDATE user_info SET user_order_addr = ?  WHERE id = ?";
    let update_user_addr_params = [address, user_id];
    conn.query(update_user_addr_sql, update_user_addr_params, (error, results, fields) => {
        if (error) {
    ...
    let add_sql = "INSERT INTO user_order(goods_content, user_id, address, create_time) VALUES (?, ?, ?, ?)";
    let sql_params = [JSON.stringify(goods_content), user_id, address, formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss')];
    conn.query(add_sql, sql_params, (error, results, fields) => {
        if (error) {
            console.log(error);
            ...
            goods.forEach((item, index, arr) => {
                ...
                conn.query(get_goods_sql, get_goods_sql_params, (error, results, fields) => {
                    if (error) {
                        ...
                        if (!results) {
                            ...
                            results = JSON.parse(JSON.stringify(results));
                            let update_goods_counts_sql = "UPDATE recommend SET counts = ?  WHERE goods_id = ?";
                            let now_counts = results[0].counts - item.counts
                            let update_goods_sql_params = [now_counts, item.goods_id];
                            conn.query(update_goods_counts_sql, update_goods_sql_params, (error, results, fields) => {
                                if (error) {
                    ...
                    //清空购物车
                    let del_cart_sql = 'DELETE FROM cart WHERE goods_id = ? and user_id = ?';
                    let del_cart_sql_params = [item.goods_id, user_id];
                    conn.query(del_cart_sql, del_cart_sql_params, (error, results, fields) => {
                        if (error) {
...
router.post('/api/get/order', (req, res) => {
    ...
    let get_orders_sql = 'SELECT * FROM user_order WHERE user_id = ? ORDER BY create_time DESC';
    let get_orders_sql_params = [user_id];
    conn.query(get_orders_sql, get_orders_sql_params, (error, results, fields) => {
        if (error) {
